...
const allControllers = controllers as Controller[];
const allAdapters = adapters as Adapter[];
...
function findController(query: string): Controller | undefined {
  const q = query.toLowerCase();
  return (
    allControllers.find((c) => c.slug === q) ||
...
function controllerMatchesAdapter(controller: Controller, adapter: Adapter): boolean {
  ...
  if (inputType === "usb" && (conn === "usb" || conn === "wireless")) return true;
  if (inputType === "bt" && conn === "bluetooth") return true;
  if (inputType === "native" && conn === "proprietary") return true;

  // Also check protocol match
  const proto = controller.protocol.toLowerCase();
  if (
    adapter.inputProtocols.some(
      (p) => p.toLowerCase() === proto
    )
  )
    return true;

  // Check supportedApps for direct adapter slug match
  if (controller.supportedApps.split(",").includes(adapter.slug)) return true;

  return false;
}

function platformNormalize(platform: string): string {
...
server.tool(
  "compatibility_check",
  "Check if a specific controller is compatible with a target platform via Joypad adapters",
  {
    controller: z.string().describe("Controller name, slug, or search term"),
    platform: z
      .string()
      .describe(
        "Target platform (PC, GameCube, N64, SNES, NES, Genesis, Saturn, Dreamcast, PCEngine, 3DO, Nuon, NeoGeo)"
      ),
  },
  async ({ controller: query, platform }) => {
    ...
    const matchingAdapters = allAdapters.filter(
      (a) =>
...
server.tool(
  "find_adapter",
  "Find adapters by input type and target output platform",
  {
    inputType: z
      .string()
    ...
    outputPlatform: z
      .string()
      .describe(
        "Target output platform (PC, GameCube, N64, SNES, NES, Genesis, Saturn, Dreamcast, PCEngine, 3DO, Nuon, NeoGeo)"
      ),
  },
  async ({ inputType, outputPlatform }) => {
    ...
    const matches = allAdapters.filter(
      (a) =>
        a.inputType.toUpperCase() === mappedInput.toUpperCase() &&
        a.outputPlatform === normalizedPlatform
    );
...
server.tool(
  "controller_info",
  "Look up detailed information about a specific controller by name, slug, or manufacturer",
  {
    query: z
      .string()
  ...
  async ({ query }) => {
    ...
    if (!ctrl) {
      ...
      const matches = allControllers.filter(
        (c) =>
...
server.tool(
  ...
  "List and filter controllers by manufacturer, platform, era, or connection type",
  {
    manufacturer: z
      ...
      .optional()
      .describe(
        "Filter by manufacturer (Nintendo, Sony, Microsoft, 8BitDo, Sega, etc.)"
      ),
    platform: z
      .string()
      .optional()
      .describe(
        "Filter by platform (Nintendo, PlayStation, Xbox, PC, Multi, Sega)"
      ),
    era: z
      .string()
      .optional()
    ...
    connectionType: z
      ...
      .optional()
      .describe(
        "Filter by connection: USB, Bluetooth, Wireless, Proprietary"
      ),
    limit: z
      .number()
      .optional()
  ...
  async ({ manufacturer, platform, era, connectionType, limit }) => {
    ...
    if (connectionType) {
      ...
      results = results.filter((c) =>
        c.connectionType.toLowerCase().includes(ct)
      );
    }
...
server.tool(
  "search_guides",
  "Search adapter build guides by name, description, input/output type, or platform",
  {
    query: z
      .string()
  ...
  async (uri) => {
    ...
    for (const ctrl of allControllers) {
      const row = platforms.map((platform) => {
        const compatible = allAdapters.some(
          (a) =>
            a.outputPlatform === platform &&
            controllerMatchesAdapter(ctrl, a)
        );
        return compatible ? "✅" : "—";
      });
